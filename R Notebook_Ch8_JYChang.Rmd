---
title: "R Notebook_Ch4.4-5.6_JYChang"
...
Here we want to add a line that represents the average murder rate for the entire country. Once we determine the per million rate  
```{r}
r <- murders %>% 
...
Rewrite the code above to abbreviation as the label through aes
```{r}
murders %>% ggplot() +
  geom_point(aes(x = population/10^6 , y = total )) + geom_text(aes(population/10^6, total, label = abb))
```
